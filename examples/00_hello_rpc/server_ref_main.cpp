...
class GreeterServiceImpl final : public HelloService::Service {
  Status SayHello(ServerContext* context, const HelloRequest* request,
                  HelloReply* reply) override {
                    std::cerr << "Greeting: " << request->name() << "\n";
    std::string prefix("Hello ");
    reply->set_greeting(prefix + request->name());
    return Status::OK;
  }

  Status SayBye(ServerContext* context, const HelloRequest* request,
                  HelloReply* reply) override {
    std::string prefix("Hello ");
    reply->set_greeting(prefix + request->name());
    return Status::OK;
