...
TEST(error, error_types) {
  ...
  EXPECT_EQ(rpc::error::invalid_argument("").code(),
            GRPC_STATUS_INVALID_ARGUMENT);
  EXPECT_EQ(rpc::error::deadline_exceeded("").code(),
  ...
  EXPECT_EQ(rpc::error::permission_denied("").code(),
            GRPC_STATUS_PERMISSION_DENIED);
  EXPECT_EQ(rpc::error::unauthenticated("").code(),
            GRPC_STATUS_UNAUTHENTICATED);
  EXPECT_EQ(rpc::error::resource_exhausted("").code(),
            GRPC_STATUS_RESOURCE_EXHAUSTED);
  EXPECT_EQ(rpc::error::failed_precondition("").code(),
